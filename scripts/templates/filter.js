function filterTemplate(data) {
  ...
  function getTag(name,type) {
    ...
    deleteActiveFilter.addEventListener("click", (e) => {
      console.log(e)
      const dropDownItems = document.querySelectorAll(`#option-${type}-container .filter-card.active`);
      console.log(dropDownItems);
      dropDownItems.forEach((item) => {
        if (item.childNodes[0].textContent === filterActiveContent.textContent) {
